...
def n_3_redundancy_check(net_temp, element_counts, timeout=900):
    ...
    element_triples = {
        "line": list(itertools.combinations(net_temp.line.index, min(3, element_counts["scaled_counts"][
            "line"]))) if not net_temp.line.empty else [],
        ...
        "sgen": list(itertools.combinations(net_temp.sgen.index, min(3, element_counts["scaled_counts"][
            "sgen"]))) if not net_temp.sgen.empty else [],
        "trafo": list(itertools.combinations(net_temp.trafo.index, min(3, element_counts["scaled_counts"][
            "trafo"]))) if not net_temp.trafo.empty else [],
        "bus": list(itertools.combinations(net_temp.bus.index, min(3, element_counts["scaled_counts"][
            "bus"]))) if not net_temp.bus.empty else [],
        "storage": list(itertools.combinations(net_temp.storage.index, min(3, element_counts["scaled_counts"][
            "storage"]))) if not net_temp.storage.empty else []
